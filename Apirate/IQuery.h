...
namespace Apirate {
	...
	public:
 		...
 		* @return The value assiciated to the header 
	 	*/
		virtual const std::string& getHeader(const std::string& header) = 0;

 		/**
 		* @brief Getter for all headers
 		* @details This methos allows the implementor to retrieve all the headers
 		...
 		* @details This methos allows the implementor to retrieve the data
 		...
 		* @details This methos allows the implementor to set the content of
 		* the query.
 		* @return Itself
	 	*/
		virtual IQuery& setData() = 0;
 		...
 		* @details this methos allows the implementor to know whether the query
